...
from trx_utils import is_string, is_integer, is_boolean, is_hex, encode_hex
...
class TransactionBuilder(object):
    ...
    def vote(self, votes: List[Tuple[str, int]], voter_address: str = None):
        ...
        for sr_address, vote_count in votes:
            if not self.tron.isAddress(sr_address):
                raise InvalidAddress("Invalid SR address provided: " + sr_address)

            if not is_integer(vote_count) or vote_count <= 0:
                raise ValueError("Invalid vote count provided for SR: " + sr_address)
    ...
    def create_smart_contract(self, **kwargs):
        ...
        owner_address = kwargs.setdefault(
        ...
        if (
            not is_integer(user_fee_percentage)
            or user_fee_percentage < 0
            or user_fee_percentage > 100
        ):
    ...
    def create_trx_exchange(
        self, token_name: str, token_balance: int, trx_balance: int, account: str = None
    ):
    ...
    def create_token(self, **kwargs):
        ...
        if not is_integer(kwargs.get("saleEnd")) or kwargs.get("saleEnd") <= sale_start:
        ...
        if (
            not is_integer(frozen_duration)
    ...
    def update_setting(
        self, contract_address, user_fee_percentage, owner_address: str = None
    ):
    ...
    def update_energy_limit(
        self, contract_address, origin_energy_limit, owner_address: str = None
    ):
        """Update energy limit.
    ...
    def check_permissions(self, permissions, _type):
        if permissions is not None:
            if (
                permissions["type"] != _type
                or not permissions["permission_name"]
                or not is_string(permissions["permission_name"])
                or not is_integer(permissions["threshold"])
                or permissions["threshold"] < 1
                or not permissions["keys"]
            ):
                return False

        for key in permissions["key"]:
            if (
                not self.tron.isAddress(key["address"])
                or not is_integer(key["weight"])
                or key["weight"] > permissions["threshold"]
                or key["weight"] < 1
                or _type == 2
                and not permissions["operations"]
            ):
                return False
